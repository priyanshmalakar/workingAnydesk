...
io.on('connection', (socket) => {
  ...
  socket.on('join', (room) => {
    ...
    // Handle existing remoteData events
    socket.on('remoteData', (data) => {
      socket.broadcast.to(room).emit('remoteData', data.data);
    });

    // Handle AV data from client
    socket.on('avData', (data) => {
      // data: { room, chunk }
      socket.broadcast.to(data.room).emit('avData', data.chunk);
    });
  ...
  // Extra remoteData outside join (optional)
  socket.on('remoteData', (data) => {
    socket.broadcast.to(data.room).emit('remoteData', data.data);
  });
});
