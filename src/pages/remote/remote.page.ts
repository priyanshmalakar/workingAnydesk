...
export class RemotePage implements OnInit, OnDestroy {
  ...
  micEnabled = false;
  camEnabled = false;
  localPreview = false; // show local preview element
  ...
  // file input handler (expects $event)
  ...
  init(id) {
    ...
    this.socketService.onNewMessage().subscribe(async (data: any) => {
      console.log('[REMOTE] 📨 Message received:', typeof data === 'string' ? (data as string).substring(0, 30) : 'signal');

      if (typeof data == 'string' && data?.startsWith('screenSize')) {
        const size = (data as string).split(',');
        this.hostScreenSize = {
          height: +size[2],
          width: +size[1],
        };
        ...
        try {
          ...
          console.error('[REMOTE] signal error', err);
        }
      }
    });
  ...
  initPeer(id) {
    ...
    // Create peer (remote is not initiator)
    this.peer2 = new SimplePeer({
      initiator: false,
      config: {
        iceServers: [
    ...
    this.peer2.on('stream', stream => {
      ...
      // start electron clipboard watcher if available
      if (this.electronService.isElectron) {
        try {
          console.log('[REMOTE] 📋 Starting clipboard monitoring');
          const clipboard = this.electronService.clipboard;
          ...
          console.warn('[REMOTE] clipboard watch failed', err);
        }
      }

      // set background blurred video
      ...
      if (video) {
        ...
        // bind listeners and store references for removal
        this.boundMouseListener = this.mouseListener.bind(this);
        this.boundMouseMoveListener = this.mouseMoveListener.bind(this);
        this.boundDblClickListener = this.mouseListener.bind(this);
        this.boundMouseUpListener = this.mouseListener.bind(this);
  ...
  // UI toggles for mic/cam
  async toggleMic() {
    this.micEnabled = !this.micEnabled;
    if (this.micEnabled) {
      await this.startLocalStream({ audio: true, video: !!this.camEnabled });
    } else {
      // if cam still on, keep local stream but remove audio track
      if (this.localStream) {
        this.localStream.getAudioTracks().forEach(t => t.stop());
        this.localStream = this._stripTracks(this.localStream, 'audio');
        this._attachLocalStreamToPeer(); // re-attach if needed
      }
      // if neither mic nor cam enabled, stop stream
  ...
  async toggleCam() {
    ...
    if (this.camEnabled) {
      await this.startLocalStream({ audio: !!this.micEnabled, video: true });
    } else {
      if (this.localStream) {
        this.localStream.getVideoTracks().forEach(t => t.stop());
        this.localStream = this._stripTracks(this.localStream, 'video');
        this._attachLocalStreamToPeer();
      }
      if (!this.micEnabled && !this.camEnabled) {
        this.stopLocalStream();
      }
  ...
  async startLocalStream(constraints: MediaStreamConstraints) {
    try {
      // request the requested tracks only
      const newStream = await navigator.mediaDevices.getUserMedia(constraints);

      // if we already had a localStream, stop its tracks and replace
      if (this.localStream) {
        // merge tracks: keep existing tracks that are not replaced
        const merged = new MediaStream();

        // add existing tracks that are not replaced by newStream
        this.localStream.getTracks().forEach(track => {
          const kind = track.kind;
          const newTracksOfKind = newStream.getTracks().filter(t => t.kind === kind);
          if (newTracksOfKind.length === 0) merged.addTrack(track);
        });

        // add new tracks
        newStream.getTracks().forEach(t => merged.addTrack(t));
      ...
      // attach to local preview element
      this.attachLocalPreview(this.localStream);

      // attach to peer (so host will receive our audio/video)
      this._attachLocalStreamToPeer();
      ...
      // revert toggles if permission denied
      if (constraints.audio) this.micEnabled = false;
      if (constraints.video) this.camEnabled = false;
      const alert = await this.alertCtrl.create({
        header: 'Media permission',
        message: 'Could not access microphone/camera. Make sure permissions are allowed.',
        buttons: ['OK'],
      });
      await alert.present();
    }
  ...
  _attachLocalStreamToPeer() {
    ...
    try {
      ...
      try {
        ...
        console.warn('[REMOTE] addStream fallback also failed', e);
      }
    }
  }

  // helper to remove a kind tracks from stream
  _stripTracks(stream: MediaStream, kindToRemove: 'audio' | 'video'): MediaStream {
    const newStream = new MediaStream();
    stream.getTracks().forEach(t => {
      if (t.kind !== kindToRemove) newStream.addTrack(t);
    });
    return newStream;
  }

  attachLocalPreview(stream: MediaStream) {
    try {
      const localVideo: HTMLVideoElement = this.elementRef.nativeElement.querySelector('#localPreview');
      if (localVideo) {
  ...
  stopLocalStream() {
    ...
    // remove local preview src
    try {
      const localVideo: HTMLVideoElement = this.elementRef.nativeElement.querySelector('#localPreview');
      if (localVideo) localVideo.srcObject = null;
    } catch (err) {}
    // attempt to remove tracks from peer's senders
    try {
      const pc: RTCPeerConnection = (this.peer2 as any)?._pc;
      if (pc) {
        pc.getSenders().forEach(sender => {
  ...
  mouseListener(event: MouseEvent) {
    ...
    const x = this.scale(event.offsetX, 0, this.videoSize.width, 0, this.hostScreenSize.width);

    const y = this.scale(event.offsetY, 0, this.videoSize.height, 0, this.hostScreenSize.height);
  ...
  mouseMoveListener(event) {
    ...
    const x = this.scale(event?.offsetX, 0, this.videoSize?.width || 1, 0, this.hostScreenSize?.width || 1);
    const y = this.scale(event?.offsetY, 0, this.videoSize?.height || 1, 0, this.hostScreenSize?.height || 1);
